    ...
    <Page size="A4" style={styles.page}>
      <View style={styles.section}>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.value}>PKR {paymentData.amount.toFixed(2)}</Text>
        </View>
        <View style={styles.row}>
          <Text style={styles.label}>Amount Pending:</Text>
          <Text style={styles.value}>PKR {5000 - parseInt(paymentData.amount.toFixed(2))}</Text>
        </View>
        <View style={styles.row}>
...
const PaymentReceipt: React.FC<{ paymentData: PaymentData }> = ({ paymentData }) => (
  <PDFDownloadLink document={<Receipt paymentData={paymentData} />} fileName="payment_receipt.pdf">
      <Button>
        Download Receipt
      </Button>
  </PDFDownloadLink>
